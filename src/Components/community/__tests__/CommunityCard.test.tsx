...
describe('CommunityCard', () => {
  it('renders title, description and member count correctly', () => {
    const {getByText} = render(<CommunityCard community={mockCommunity} onPress={jest.fn()} />);

    expect(getByText('React Native Developers')).toBeTruthy();
    expect(
      getByText(
        'A global community of React Native developers sharing tips, news and job openings.',
      ),
    ).toBeTruthy();
  ...
  it('calls onPress with community id when clicked', () => {
    ...
    const {getByRole} = render(<CommunityCard community={mockCommunity} onPress={onPressMock} />);
  ...
  it('displays the Private badge when community is private', () => {
    ...
    const {getByText} = render(<CommunityCard community={privateCommunity} onPress={jest.fn()} />);
  ...
  it('displays the Joined badge when user is a member', () => {
    ...
    const {getByText} = render(<CommunityCard community={joinedCommunity} onPress={jest.fn()} />);

    expect(getByText('Joined')).toBeTruthy();
  });

  it('does not display Private or Joined badges by default', () => {
    const {queryByText} = render(<CommunityCard community={mockCommunity} onPress={jest.fn()} />);
